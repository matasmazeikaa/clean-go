...
const PricingSection = ({ startingCleaningType, startingService }) => {
	...
	const [service, setService] = useState(startingService || CLEANING_SERVICE_BUTO_VALYMAS);
	...
	const adjustPrice = useCallback(() => {
		...
		if (cleaningType === CLEANING_TYPE_PERIODINIS || cleaningType === CLEANING_TYPE_POSTATYBINIS) {
			resetPriceRange();
		}
	...
	const renderCleaningTypes = () => (
		<>
			<p className="pricing-section__pricer-title body-2">Pasirinkite valymo tipą:</p>
				...
				{CLEANING_TYPES.map((type) => (
					<button
						...
						className={cx({
							...
							'pricing-section__select-container--active': type === cleaningType,
						})}
					>
						{type}
					</button>
	...
	const renderServices = () =>
		cleaningType !== CLEANING_TYPE_LANGU_VALYMAS && (
			<>
				<p className="pricing-section__pricer-title body-2">Pasirinkite paslaugą:</p>
				<div className="pricing-section__select-container-wrapper">
						...
						<div key={cleaningService.title}>
							<button
								...
								className={cx({
									...
									'pricing-section__select-container--active': service === cleaningService.title,
								})}
								onClick={updateService(cleaningService.title)}
							>
								<img src={cleaningService.icon} alt="" className="pricing-section__select-icon" />
							</button>
		...
		<>
			...
			<div className="pricing-section__select-container-wrapper slidecontainer">
				<input
					...
					id="myRange"
				/>
				<input className="slider-input" type="text" value={area} onChange={handleAreaChange} />
			</div>
		</>
	);
		...
		cleaningType === CLEANING_TYPE_PERIODINIS && (
			<>
				...
				<div className="pricing-section__frequency-container">
					<label htmlFor="1-frequency" className="pricing-section__radio-btn-container">
						<input
							id="1-frequency"
					...
					<label htmlFor="2-frequency" className="pricing-section__radio-btn-container">
						<input
							id="2-frequency"
							className="pricing-section__radio-btn-input"
							type="radio"
